...
function shouldSkipPatch() {
  if (process.env.RAILWAY_ENVIRONMENT || process.env.CI) return true;
  const root = path.join(__dirname, '..');
  if (!fs.existsSync(path.join(root, 'android'))) return true;
  if (fs.existsSync('/.dockerenv')) return true;
  return false;
}

if (shouldSkipPatch()) process.exit(0);
...
try {
  ...
  console.warn('patch-capacitor-java: ignorado (ambiente sem Android ou erro):', err.message);
  process.exit(0);
}
